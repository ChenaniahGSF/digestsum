...
static void SHA512Encode(unsigned char *output, unsigned long long *input, unsigned long long len) {
...
static void SHA512Decode(unsigned long long *output, const unsigned char *input, unsigned long long len) {
    unsigned long long i = 0, j = 0;
    while (j < len)
    {
...
void sha512_init(void *ctx) {
    SHA512_CTX* context = (SHA512_CTX*)ctx;
	context->count[0] = 0;
	context->count[1] = 0;
...
void sha512_update(void *ctx, const unsigned char *input, size_t inputlen) {
    SHA512_CTX* context = (SHA512_CTX*)ctx;
...
void sha512_final(void *ctx, unsigned char digest[64]) {
    SHA512_CTX* context = (SHA512_CTX*)ctx;
