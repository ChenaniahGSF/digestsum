...
{
    sha256_context_t* _ctx = (sha256_context_t*)ctx;
...
void sha256_update(void *ctx, const unsigned char *in, size_t len)
{
    ...
    uint32_t r = (_ctx->count[1] >> 3) & 0x3f;
    ...
    if ((_ctx->count[1] += bitlen1) < bitlen1) {
        _ctx->count[0]++;
    }

    _ctx->count[0] += bitlen0;

    /* Handle the case where we don't need to perform any transforms */
    if (len < 64 - r) {
        memcpy(&_ctx->buf[r], in, len);
        return;
    }
    ...
    memcpy(&_ctx->buf[r], src, 64 - r);
    sha256_transform(_ctx->state, _ctx->buf);
    src += 64 - r;
    len -= 64 - r;

    /* Perform complete blocks */
    while (len >= 64) {
        sha256_transform(_ctx->state, src);
        src += 64;
        len -= 64;
    }

    /* Copy left over data into buffer */
    memcpy(_ctx->buf, src, len);
}
...
void sha256_final(void *ctx, unsigned char digest[32])
{
    ...
    /* Write the hash */
    be32enc_vect(digest, _ctx->state, 32);

    /* Clear the context state */
    memset((void *) _ctx, 0, sizeof(*_ctx));
}
