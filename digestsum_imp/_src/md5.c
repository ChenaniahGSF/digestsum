...
{
    MD5_CTX* _context = (MD5_CTX*)context;
    _context->count[0] = 0;
    _context->count[1] = 0;
...
void md5_update(void *context, const unsigned char *input, size_t inputlen)
{
    ...
    index = (_context->count[0] >> 3) & 0x3F;
    partlen = 64 - index;
    _context->count[0] += inputlen << 3;

    if(_context->count[0] < (inputlen << 3))
        _context->count[1]++;
    _context->count[1] += inputlen >> 29;

    if(inputlen >= partlen)
    {
        memcpy(&_context->buffer[index], input,partlen);
        MD5Transform(_context->state, _context->buffer);

        for(i = partlen; i+64 <= inputlen; i+=64)
            MD5Transform(_context->state, &input[i]);
...
void md5_final(void *context, unsigned char* digest)
{
    ...
    MD5Encode(bits, _context->count, 8);
    md5_update(_context, PADDING, padlen);
    md5_update(_context, bits, 8);
    MD5Encode(digest, _context->state, 16);
}